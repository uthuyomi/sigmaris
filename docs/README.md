# Docs

この `docs/` は、ShiftPilotAI の設計・判断・運用記録をまとめる場所だよ。  
Notion にそのまま移しやすいように、親ページに相当する index と、カテゴリごとの詳細ページに分けてある。

## Notion 用の親ページ候補

- [project-overview.md](/d:/souce/ShiftPilotAI/docs/project-overview.md)
- [notion-page-map.md](/d:/souce/ShiftPilotAI/docs/notion-page-map.md)
- [requirements/README.md](/d:/souce/ShiftPilotAI/docs/requirements/README.md)
- [design/README.md](/d:/souce/ShiftPilotAI/docs/design/README.md)
- [decisions/README.md](/d:/souce/ShiftPilotAI/docs/decisions/README.md)
- [operations/README.md](/d:/souce/ShiftPilotAI/docs/operations/README.md)

## backend を含む重要ページ

- [backend-api-architecture.md](/d:/souce/ShiftPilotAI/docs/design/backend-api-architecture.md)
- [backend-chat-stream.md](/d:/souce/ShiftPilotAI/docs/design/backend-chat-stream.md)
- [backend-endpoint-catalog.md](/d:/souce/ShiftPilotAI/docs/design/backend-endpoint-catalog.md)
- [frontend-architecture.md](/d:/souce/ShiftPilotAI/docs/design/frontend-architecture.md)
- [system-architecture.md](/d:/souce/ShiftPilotAI/docs/design/system-architecture.md)
- [supabase-schema.md](/d:/souce/ShiftPilotAI/docs/design/supabase-schema.md)
- [supabase-migration-map.md](/d:/souce/ShiftPilotAI/docs/design/supabase-migration-map.md)
- [settings-and-preferences.md](/d:/souce/ShiftPilotAI/docs/design/settings-and-preferences.md)
- [backend-refactor-and-intent-routing.md](/d:/souce/ShiftPilotAI/docs/decisions/2026-03-31-backend-refactor-and-intent-routing.md)
- [2026-03-31-comprehensive-log.md](/d:/souce/ShiftPilotAI/docs/operations/2026-03-31-comprehensive-log.md)
- [2026-03-31-documentation-audit.md](/d:/souce/ShiftPilotAI/docs/operations/2026-03-31-documentation-audit.md)

## 運用ルール

- 1テーマ1ファイルを基本にする
- 判断は `decisions/`
- 実装構成は `design/`
- 日々の履歴は `operations/`
- ユーザー要件や前提は `requirements/`

## 補足

既存の古いメモや途中経過のページも残してある。  
今回追加した総覧ページは、それらを上から追いやすくするための導線として使う想定だよ。
