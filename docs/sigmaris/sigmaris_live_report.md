...
## Redesign-4: ツール詳細の充実 ＋「一時停止しても情報量が豊富」なレイアウト（最終段階、2026-07-21）

Sigmaris Live サイドバー再設計の最終段階。当初提案の「動画を一時停止したとき、メモリ検索結果・判断理由・モデル選択・Drive・Tool・Safety・Timeline が全て見えている」という状態を、**新しい情報を足すのではなく、Redesign-1〜3 の要素を1画面に綺麗に収めるレイアウトの最終調整**として仕上げた。**変更は `live-sidebar-panel.tsx` の1ファイルのみ**（`/live`・葉コンポーネント・共有 `live-event-detail-panel.tsx`・バックエンドは不変更。実データのみ・捏造なし）。

### 1. ツール呼び出しの詳細の実装内容

- **読みやすいツール名**: `TOOL_NAME_LABELS` で内部の関数名（`create_google_calendar_events` 等）を平易な日本語（`カレンダー登録` 等、11種）へ変換（Redesign-2 の平易化原則の継続。未知名はそのまま＝捏造しない）。
- **成否＋所要時間をインライン表示**: ツール行を `カレンダー登録(成功)・320ms` の形に（`tool_name`・`ok`・`elapsed_ms` の実データ）。
- **1ターン内の複数ツールを区別**: `tool_call_finished` を1件ずつ別行（`tool-0`/`tool-1`…）で表示（Redesign-2 で確立済みの構造を踏襲・維持）。各行クリックで、既存の `LiveEventDetailPanel`（Live-6・マスキング撤廃済み）を展開し、**実際の引数**を表示する。
- 共有 `live-event-detail-panel.tsx` は /live も使うため不変更。ネストした引数が `[object Object]` になる点は据え置き（§5 申し送り。大半のツール引数はフラットな文字列のため実害は小さい）。

### 2. レイアウトの最終調整内容

- **常時表示（一時停止しても読める中核）**: ①主役の段階的な大きなテキスト（Redesign-1・不変更）、②「処理の流れ」、③「ログ（ターンごと）」。③には各ターンの **メモリ・文脈（判断理由）・モデル・Drive・Safety・ツール** の内訳が並ぶ＝**このターンの Timeline**。frozen frame（一時停止）で、これらが一望できる。
- **折りたたみの使い分け**（依頼書「常に全て開いた状態にする必要はない」）: 二次情報の **「メトリクス（所要時間）」を `CollapsibleSection` で既定折りたたみ**に。読みたい人だけ展開する。中核（Stage/処理の流れ/ターンごとの内訳）は常時開いたまま。
- **並び替え**: 「ログ（ターンごと）」を「メトリクス」より上へ移動し、frozen frame の主要情報（各区分の内訳）を上部に集約。
- **フォント・余白の微調整**: セクション間隔を `space-y-6` → `space-y-5`、キャプションの上マージンを詰め、狭幅でも密度を保ちつつ崩れないようにした。

### 3. Timeline の扱いについての判断根拠

- 提案の `Timeline` は、**記憶画面の `/memory?tab=timeline`（＝記憶の時間変遷）とは別文脈**で、**「このターンでの処理の時系列」**を指すと解釈した。
- これは **Redesign-2 の `LiveTurnLog`（ターン単位のログ）が既に実質的に担っている**：各ターンを `invocation_id` でグルーピングし、時刻ヘッダの下に処理区分（意図→モデル→記憶→文脈→Drive→Safety→ツール→生成）を時系列的な順序で並べている。したがって **Timeline の新規実装は不要**と判断した（依頼書が許容する判断）。frozen frame で「このターンで何がどの順に起きたか」が読める＝提案の Timeline 要件を満たす。
...
| `npx next build` | 成功（`/live` 含む全ルート健在） |
| `pytest tests/`（backend 16件） | 16 passed |
| `/live`・葉コンポーネント・`live-event-detail-panel.tsx`・バックエンドの不変更 | 差分なし（`git status` で確認） |
| Redesign-1〜3 の中核（`LiveStageDisplay`/`LiveTurnLog`/`deriveTurnRows`/Model・Drive・Safety）の非破壊 | ファイル内に存置・機能維持（grep 確認） |
| frozen frame での情報密度・可読性 | コード上、中核常時表示＋メトリクス折りたたみ＋ツール平易化で成立。実ブラウザ目視は未実施（実データ・SSE が必要） |

**変更ファイル**: `frontend/src/components/live/live-sidebar-panel.tsx` の1ファイルのみ。

---

## Sigmaris Live サイドバー 全面再設計（Redesign-1〜4）完了サマリー

| 段階 | 内容 | 主な成果 | 変更範囲 |
|---|---|---|---|
| R-1 | マスキング撤廃 ＋ 段階的な大きなテキスト | `/chat` は本人限定のため詳細のマスキングを撤廃。実イベントを英語の短い行で大きく段階表示（`LiveStageDisplay`、演出禁止を厳守） | `live_detail_masking.py`／`live-sidebar-panel.tsx` |
| R-2 | 常時ログの密度向上 | ログを1ターン単位・区分別（意図/記憶/文脈/ツール/生成）・平易な言葉へ（`LiveTurnLog`/`deriveTurnRows`。専門用語を出さない） | `live-sidebar-panel.tsx` |
| R-3 | Model・Drive・Safety | バックエンドのイベントに実フィールドを追加（Model=応答生成モデルのティア、Drive=is_proactive、Safety=ガード発火の痕跡）。fire-and-forget で応答速度に非影響 | `chat.py`／`orchestrator/service.py`／`live-sidebar-panel.tsx` |
| R-4 | ツール詳細 ＋ 一時停止密度 | ツール名の平易化＋所要時間、複数ツールの区別、メトリクスの折りたたみ、frozen frame で全区分が読めるレイアウト | `live-sidebar-panel.tsx` |

**全体を通じた設計の一貫性**:
- **実データのみ・演出禁止**: 全段階で、実際に発生したイベントの実データだけを扱い、疑似プログレス・演出的遅延・値の捏造を一切行っていない。データが無い場合は「該当なし＝表示省略」を貫いた（Drive の is_proactive が常時 False な点も正直に扱った）。
- **既存資産の再利用**: 葉コンポーネント（process-flow/metrics/event-log）・`useLiveEvents`・`process-steps`/`metrics`・`LiveEventDetailPanel` は再利用のみで、`/live`（フルスクリーン）と共有。サイドバー固有の表示だけを `live-sidebar-panel.tsx` に集約した。
- **`/live` と `/chat` の不可侵**: 全段階で `/live` 独立ページ・葉コンポーネントを不変更、`/chat` の常時ダーク保護（第二段階）も維持。
- **応答速度の最優先**: R-3 のイベント拡張はフィールド追加＋条件付き fire-and-forget emit のみで、新規 LLM/DB/同期処理はゼロ。

**残っている懸念事項（総まとめ）**:
1. **実ブラウザ・実SSE 目視が全段階で未実施**（本環境に実 Supabase 認証・実データ・SSE が無いため）。特に、段階表示の遷移、ターン分割・区分の見え方、Model/Drive/Safety の実データ反映、frozen frame の密度・可読性は、本番反映後の実機確認を強く推奨。加えて、Live のイベントが流れるには**本番バックエンドに `/live/stream`・`/live/detail` が反映済み**である必要がある（未反映だと 404）。
2. **`invocation_id` と `message_id` の相関ギャップ（既存の未解決事項）**: 1ユーザーターンで、orchestrator 発イベント（`invocation_id`＝Memory/Context/Drive/Safety）と chat.py 発イベント（`message_id`＝Intent/Model/Tools/Generation）が別 id になり、`LiveTurnLog` では2グループに分かれて表示される。統合には `agent_chat_stream()`（共有ホットパス）のシグネチャ変更が必要でスコープ外（chat.py:1020 付近のコメント）。「1ユーザーターン=1グループ」に統合したい場合の要対応事項。
3. **Drive の具体名**（Knowledge Gap/Mastery/Coherence）と proactive ターン自体は、`goal_proposal`（S-2）が未配線のため現状発生しない（Drive は常に該当なし）。S-2 配線時に、発火した Drive を live イベントへ載せれば具体表示できる。
4. **Safety の網羅性**: 現状は呼び名ガード・事実整合ガードのみ。B11 の棄権（文脈区分で一部可視化済み）・dissent・citation_audit 等の痕跡は対応イベントが無く、必要なら同じ fire-and-forget パターンで追加可能。
5. **共有 `live-event-detail-panel.tsx`**: マスキング撤廃後の注記文言（「…マスキングして表示します…」）が実態と不一致、およびネスト引数の `[object Object]` 表示。/live 共有ファイルのため、/live への影響を確認の上まとめて更新するのが安全。
6. **改行コード（R-3 の副作用）**: `orchestrator/service.py` が R-3 の編集で LF→CRLF に変換された（リポジトリ多数派の CRLF に整合した副作用。コードは正常）。据え置き済みだが、`.gitattributes` 導入による恒久的な正規化は別タスクとして検討余地。
