...
# 第五段階（Sigmaris Live のサイドバー化）実施報告

**実施日**: 2026-07-21
**ブランチ**: `design-unify-live-sidebar`（`main` の第四段階マージ後 `160d241` から作成）
**目的**: 当初からの要望「Sigmaris Live を `/chat` の右サイドバーとして開閉表示し、メッセージ送信直後に内部処理をその場で見られるようにする」を実現する（デザイン統一の最終段階）。

## 29. サイドバーの UI 設計・実装詳細

- **開閉トグル**: `/chat` ヘッダー（`chat-workspace.tsx` のスレッドタイトル行）右端に、既存のダークなアイコンボタン様式（`hover:bg-[#2f2f2f]`）で **`ActivityIcon` のトグル**を追加。押下で `liveOpen` を切替。既定は**閉**（会話が主役・Live は補助という依頼書の位置づけに沿う。開時はボタンを `bg-[#2f2f2f]` でハイライト）。
- **開閉パネル(常時表示でない)**:
  - **デスクトップ(md+)**: 会話ペインの右に**インライン列**を追加し、左の会話履歴サイドバーと同じ**幅トランジション**（`w-[400px]`↔`w-0`、`transition-[width]`、`overflow-hidden`＋内側固定幅で反流を防止）で開閉。区切りは `border-l border-white/10`。
  - **モバイル**: 右からの**ドロワーオーバーレイ**（`fixed inset-0` + `ml-auto` の右寄せパネル、`bg-black/55` 背面、既存キーフレーム `swipe-in-from-right` を再利用してスライドイン、背面タップで閉じる）。左ドロワーと同様、開時は背面スクロールを停止。
- **パネル本文**: 新規 `components/live/live-sidebar-panel.tsx`（**表示専用**、データソース非依存）。ヘッダー（Σ＋「Sigmaris Live」＋接続状態インジケータ＋閉じるボタン）＋説明文＋**既存の `LiveProcessFlow`・`LiveMetrics`・`LiveEventLog` を縦積み**でそのまま再利用。配色は `/chat` 左サイドバーと同じ `bg-[#171717]` 基調。
- **`/chat` の既存デザイン・常時ダーク保護との整合**: 追加はヘッダーのアイコンボタン1つと右パネルのみで、会話ペイン・左サイドバーの見た目は不変。パネルは ChatWorkspace の `.dark` サブツリー内に描画され、葉コンポーネントも元々ダーク hex のため、第二段階の常時ダーク保護（`.dark`＋`theme="dark"`）に自然に乗る（light テーマでもダーク）。`chat/page.tsx`・保護コードは**不変更**。

## 30. リアルタイム性の実現方法（既存 `useLiveEvents()` の再利用）

- `ChatWorkspace` で **`useLiveEvents(undefined, { enabled: liveOpen })` を1回だけ呼び**、返る `events`/`status` を props でパネルへ渡す。`/api/live/stream`（Next ルート→backend `/api/agent/live/stream` を agent 認証で中継）への SSE から、意図分類・記憶検索・応答生成・ツール実行のイベントが届く。
- パネルを開いた状態で `/chat` にメッセージを送ると、そのターンの処理イベントが `events` に流れ込み、`LiveProcessFlow`（実行中スピナー→完了）・`LiveMetrics`（所要時間等）・`LiveEventLog`（時刻・処理名・結果、クリックで詳細）が**リアルタイムに更新**される。
- **Live-1 の「演出の禁止」原則・既存のイベント設計は一切変更していない**（新規イベント・疑似プログレスを追加していない）。表示は既存の葉コンポーネントそのまま。

## 31. `/live`(独立ページ)とのコード共有方法

- **`/live` ページ（`app/live/page.tsx` + `LiveDashboard`）は削除せず不変更**。デモモード（`?demo=1`）のフルスクリーン表示（X発信・動画撮影用）という独自用途を保持。
- **葉コンポーネント（`LiveProcessFlow`/`LiveMetrics`/`LiveEventLog`）・`useLiveEvents`・`process-steps`/`metrics`/`types`/`live-event-detail-panel` は、`/live`(LiveDashboard) とサイドバー(LiveSidebarPanel) の両方で共有**。両者は「1つのデータソース(events)を複数表示コンポーネントへ配る」という Live-3 の設計を踏襲し、**composе するレイアウトだけが異なる**（フルスクリーン vs 右サイドバー）。葉コンポーネントの重複実装はゼロ。新規は `LiveSidebarPanel`（薄い composition のみ）だけ。

## 32. SSE 接続の開閉制御の実装内容

- **閉じている間は SSE を張らない**（要件5）。実装は既存 `useLiveEvents` の **`enabled` オプション**を使うのみ:
  - `useLiveEvents(undefined, { enabled: liveOpen })`。`enabled=false`（閉）のとき `useEffect` が早期 return し、**`EventSource` を一切生成しない**（`use-live-events.ts` の該当ロジック）。
  - `liveOpen` が `false→true` になると effect が再実行され `EventSource` を接続、`true→false` で cleanup（`source.close()`）が走り切断。
- フックは ChatWorkspace で**1回だけ**呼ぶため、デスクトップ列・モバイルドロワーの両描画があっても**SSE 接続は常に最大1本**（Live-3 の「1接続・複数表示」設計を踏襲）。
...
| サイドバー開閉／送信後のリアルタイム表示／閉時の非接続／light-darkでの/chat維持 | コード上、トグル state・`enabled=liveOpen`・葉コンポーネント再利用・`.dark`サブツリーで成立（ビルド・型検査通過）。実ブラウザ目視は未実施（実データ・SSEが必要） |

**変更ファイル**: `frontend/src/components/chat-workspace.tsx`（トグル＋右パネル＋1回の useLiveEvents）、`frontend/src/components/live/live-sidebar-panel.tsx`（新規・表示専用）。**`/live`・葉コンポーネント・`/chat`保護は不変更**。

---

# デザイン統一（Step 1〜5）完了サマリー

| Step | 内容 | 主な成果 | 状態 |
|---|---|---|---|
| 1 | トークン整理＋共有UI抽出 | 4ページの hex をトークン化、`components/shared/` に共有部品6種を抽出（正味 −177行） | 完了・main |
| 2 | light/dark 本格対応 | `:root`=light/`.dark`=dark、body/html をテーマ追従化、**`/chat` を二重保護で常時ダーク** | 完了・main |
| 3 | ナビ一本化 | `/chat` サイドバーに主要導線を追加、`/live` を隠しページとして明記 | 完了・main |
| 4 | 記憶ページ統合 | `/memory`・`/timeline`・`/admin/memory` を `/memory` の3タブ（現在地/変遷/生データ）へ統合、旧URLはリダイレクト（正味 −545行） | 完了・main |
| 5 | Sigmaris Live サイドバー化 | `/chat` 右に開閉式 Live パネル、送信直後に内部処理をリアルタイム表示、閉時は SSE 非接続 | 完了・main |

**全体を通じた設計の一貫性**:
- **既存資産の最大活用**: 新規トークン体系・新規イベント設計・葉コンポーネントの再実装を一切せず、`globals.css` のトークン・`components/shared/`・Live の既存フック/コンポーネントを流用。
- **`/chat` の不可侵**: 全段階を通じて `/chat` の視覚デザインとダーク固定を維持（Step2 の二重保護、Step5 の追加は補助パネルのみ）。
- **重複の解消**: 各ページにコピーされていた UI 部品（Step1）、散在していた記憶ページ（Step4）、Live のフルスクリーン/サイドバー表示（Step5）を、いずれも共有・単一化した。

**残っている懸念事項（総まとめ）**:
1. **ブラウザ目視が全段階で未実施**（本環境は実 Supabase 認証・実データ・SSE を持たないため）。特に **(a) light テーマでの `/chat`（ツールチップ含む）のダーク維持、(b) 記憶タブの light 表示、(c) Live サイドバー開時のリアルタイム更新・閉時の非接続** を、本番反映後に実機確認することを強く推奨。
2. **`/settings` は独自の light 実装**（stone/light＋`.settings-page` 補正）のままで、トークン方式と二重管理。将来トークンへ寄せると一貫性が増す。
3. **Recharts の JS プロパティ色・`live/*` 内部の一部 hex** は据え置き（Step1 §6.2）。light テーマでチャートの軸・ツールチップがダーク前提のまま。CSS変数を computed style 経由で読む対応が将来課題。
4. **Live メトリクスのサイドバー内レイアウト**: `LiveMetrics` は `sm:grid-cols-4`（ビューポート基準）のため、幅400pxのサイドバーではやや詰まって見える可能性（機能は成立）。将来コンテナクエリ等で最適化余地。
5. **`/admin/memory` の生データタブ**: 統合後はタブとして誰でも到達可能（内容の専門性は注記で明示）。開発者以外に隠したい要件が出た場合はタブ出し分けを検討。
6. **フロントに自動テストが無い**（型検査・lint・build のみ）。デザイン/ナビ/タブ/パネルの回帰は現状ブラウザ手動確認に依存。将来 Playwright 等の導入余地。

---

*第五段階で Sigmaris Live を `/chat` の右サイドバーへ統合し、デザイン統一 Step 1〜5 を完了した。全段階で `/chat` の見た目とダーク保護を維持し、既存資産を再利用して重複を解消した。ブラウザ目視のみ本環境では未実施のため、本番反映後の最終確認を推奨する。*

---

# 追補: Sigmaris Live サイドバーのデザイン全面修正（狭幅前提での組み直し、2026-07-21）

**ブランチ**: `live-sidebar-redesign`（`main` の第五段階マージ後 `bb597c4` から作成）
**背景**: 第五段階の初版サイドバーは、`/live`（フルスクリーン用）の葉コンポーネント（`LiveProcessFlow`/`LiveMetrics`/`LiveEventLog`）を**そのまま幅だけ狭めて縦積み**していたため、(1) メトリクスが「カードがびっしり並ぶ」ダッシュボード的な見た目になり、(2) 長いラベルが不自然に折り返され読みにくかった。本追補で、**サイドバー版（`components/live/live-sidebar-panel.tsx`）のみ**を、幅≒400px を前提に「カード」から「リスト/テキストの流れ」へ根本的に組み直した。

## 34. メトリクス表示の再設計内容

- **カード群（`grid ... rounded-2xl border bg-[#212121] p-4` の箱）を全廃**し、**1行1指標の定義リスト**に変更（左にラベル、右に値、必要なら下に muted の補足1行）。「カードがびっしり」の印象を解消（要件1）。
- **表示情報を絞り込み**（狭幅に全部詰め込まない・依頼書の方針）:
  - 各処理: `直近の所要時間`（例 `1750ms`）＋ muted 補足 `平均 1600ms・直近N件`。
  - ツール実行: `件数`（例 `3件`）＋ muted 補足 `成功 2・失敗 1`。
  - **データのある処理のみ表示**（`lastElapsedMs === null` の処理は出さない）。全て無ければ「まだ計測データがありません」。
  - 初版にあった「即時判定/LLM判定」の内訳カードは**サイドバーでは省略**（/live には残る）。
- 計算は共有の `computeStepMetrics`/`computeToolCallMetrics`（`metrics.ts`）を再利用。**新しいデータ収集・新イベントは無し**。

## 35. ラベルの簡潔化内容

- 「意図分類・直近の所要時間」→ **「意図分類」**（値 `1750ms` を右揃え）のように、ラベルは処理名のみへ短縮。値は `font-mono tabular-nums` で右揃えにし、**折り返しを起こさない**レイアウトにした（要件2）。
- ログの処理名も短縮: 「意図分類 ・ 終了」の体裁は保ちつつ、ツールは「ツール・開始/終了」に短縮。時刻は `font-mono text-[11px]` で右端に小さく。

## 36. 処理の流れ・ログ表示の調整内容

- **処理の流れ**: 横並びのカード（`flex-wrap` の箱）から、**縦並びの簡潔なリスト**へ（`点(size-6) ＋ ラベル ＋ 右端に状態テキスト`）。状態は「実行中…／完了／待機中」に集約し、長い結果要約は流れには出さず**ログ側に委ねる**（狭幅で綺麗に収める・要件2）。点は既存同様、active=紫スピナー/done=緑チェック/idle=淡い点。計算は共有の `computeStepStates` を再利用。
- **ログ**: 罫線の濃いテーブル（`<table>` + `border`）から、**区切り線の薄い縦リスト**（`divide-y divide-white/[0.06]`、箱なし）へ。1件を「1行目: 処理名＋（詳細可なら ▼）＋右端に時刻／2行目: 結果(muted・折り返し可)」の積み重ねにし、狭幅の可読性を確保。クリック（Enter/Space対応）で **既存の `LiveEventDetailPanel` を展開**（記憶検索・ツール実行の詳細、`/api/live/detail`）。ラベル整形は `PROCESS_STEPS` から導出する小ヘルパを本ファイル内に局所化（葉コンポーネント側の未export整形関数を共有するために /live 用ファイルへ手を入れないための判断）。

## 37. テスト結果（Live サイドバー修正）
...
| `npx next build` | **成功**（`/live` 含む全ルート健在） |
| `pytest tests/`（backend 16件） | **16 passed** |
| `/live`（独立ページ）の不変更 | `app/live/`・`LiveDashboard`・葉3コンポーネント（metrics/process-flow/event-log）・`live-event-detail-panel` は**差分なし**（`git status` で確認）。要件3充足 |
| サイドバーの狭幅表示 | コード上、カード全廃・リスト化・ラベル短縮で成立（ビルド・型検査通過）。実ブラウザ目視は未実施（実データ・SSE が必要） |

**変更ファイル**: `frontend/src/components/live/live-sidebar-panel.tsx` の**1ファイルのみ**。

## 38. 気づいた懸念点

1. **実ブラウザ目視が未実施**（本環境の制約）。狭幅での見た目（特に長い結果要約の折り返し・詳細パネル展開時の収まり）は、本番反映後に実機確認を推奨。
2. **ログの結果要約は元の `summarizeResult` をそのまま使う**ため、記憶検索の要約等は文言が長め（例「記憶3件(確信あり)・複数の観点に分解 ・ 320ms」）。狭幅では2〜3行に折り返す。要約自体の短縮は葉コンポーネント（/live）と共有の `process-steps.ts` に手を入れることになり /live に影響するため、本タスクでは行っていない。将来サイドバー専用のより短い要約が必要なら、`summarizeResult` とは別の短縮版を config に足す形が安全。
3. **ラベル整形ヘルパの局所的重複**: ログの処理名整形（`STARTED_LABELS` 等）を、/live 用 `live-event-log.tsx` を触らないために本ファイルへ小さく再実装した。将来 `live/event-log-format.ts` のような共有モジュールへ抽出すれば重複を解消できるが、その際は `live-event-log.tsx`（/live 使用）も同モジュール参照へ変える必要があり、/live への影響評価が前提になる。
4. **メトリクスの情報量**: サイドバーでは即時判定/LLM 内訳等を省いた。より詳細に見たい場合は `/live`（フルスクリーン）を使う、という役割分担にした。

---

*Live サイドバーを狭幅前提で「カード → リスト/テキストの流れ」へ組み直した。変更は `live-sidebar-panel.tsx` の1ファイルのみで、`/live`（フルスクリーン）・葉コンポーネントは不変更。共有の計算ロジックと詳細パネルのみ再利用した。*
