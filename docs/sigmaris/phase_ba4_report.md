...
## 20. 2026-07-16 追補: 絶対日時の常時表示への変更

### 20.1 背景

19章の修正で日時表示は正常に動作するようになったが、海星さんから「相対時間(『3日前』等)のみが表示されており、絶対日時(『2026年◯月◯日 ◯時◯分』)が常に見える状態にしてほしい。ホバー時のみの表示ではなく常時表示にし、証拠として残せるという当初の目的を踏まえ、曖昧さのない絶対日時を優先してほしい」という追加修正依頼を受けた。

### 20.2 対応内容

`MessageTimestamp`(`thread.tsx`)の表示条件・表示内容を変更した。

- **表示の主体を「相対時間」から「絶対日時」へ変更**: 従来は`formatRelativeTime()`の結果が無ければ何も表示しない設計だったが、`formatAbsoluteDateTime()`の結果を主軸にし、それが取得できた場合に常に表示する形にした。
- **`title`属性(ホバー時のみの表示)を廃止**: 絶対日時は常にテキストとして画面に表示されるため、ホバー専用の補助情報という位置づけ自体が不要になった。
- **相対時間は絶対日時への括弧書きの併記に変更**: 「2026年7月16日 10:00(3日前)」のように、絶対日時の直後に丸括弧で相対時間を付記する。相対時間が算出できない場合(理論上ほぼ発生しないが、念のための防御)は絶対日時のみを表示する。
- **絶対日時の書式を「年/月/日」の漢字区切りへ変更**: 従来は`/timeline`ページの`formatDate()`と揃えた「2026/07/16 10:00」というスラッシュ区切りだったが、依頼書の例示(「2026年◯月◯日 ◯時◯分」)に合わせ、`Intl.DateTimeFormat("ja-JP", { month: "long", day: "numeric", ... })`により「2026年7月16日 10:00」という漢字区切りの表記へ変更した(`format-time.ts`)。**判断根拠**: 「証拠として残せる」という目的においては、スラッシュ区切りよりも年月日が明示される漢字表記の方が、将来この記録を読む人にとって曖昧さがないと判断し、`/timeline`ページとの書式の一貫性よりも依頼内容を優先した。
- **相対時間のフォーマットを`style: "narrow"`へ変更**: `Intl.RelativeTimeFormat("ja-JP")`はデフォルト(`style: "long"`または`"short"`)だと数値と単位の間に半角スペースが入り「3 日前」のような表記になることが分かった(実際に`node`で出力を確認)。括弧書きで絶対日時に併記する用途では不自然なため、スペースの入らない`narrow`スタイル(「3日前」)に変更した。

### 20.3 テスト結果

- `node`で`Intl.DateTimeFormat`/`Intl.RelativeTimeFormat`の実際の出力を直接確認し、依頼書の例示通り「2026年7月13日 20:40(3日前)」のような形式になることを検証した。
- `npx eslint`(`thread.tsx`・`format-time.ts`)・`npx tsc --noEmit`(プロジェクト全体)・`npm run build`(本番ビルド)のいずれも成功。
- バックエンド側の変更は無い。

**実ブラウザでの目視確認は行っていない**(19章と同じ制約)。デプロイ後の見え方の確認を運用者側にお願いしたい。
