...
| 20 | 【2026-07-10発見、11章で修正済み】B2(`experience_layer.py::consolidate_episodic_memory()`)にmemory_extractor.py/decision_log.pyと同型の欠陥（既存事実を見せずに新規category/keyを発明）を確認し修正。同型の欠陥がB9(`knowledge_graph.py`)・B14(`extract_preference_patterns()`)・B16(`goal_alignment.py`)にも存在することを確認し、11章で全て修正済み | 本タスク10.3〜10.4節・11章（コード確認・修正済み） | 中→修正済み | 設計上の欠陥（横展開、計6箇所の同型欠陥） | 対応済み。効果は数週間の運用後に確認する必要あり（11.5節） |
...
## 11. B9・B14・B16の固定リスト欠陥、一括修正（2026-07-10）

### 11.1 設計上の重要な気づき: B9・B14・B16は、B1検索だけでは前5件と同型の解決にならない

実装に着手する前に、前5件の修正（memory_extractor.py、decision_log.py×2、experience_layer.py）と、今回対象の3件との間に構造的な違いがあることに気づいた。判断根拠として明記する。

前5件は、いずれも**書き込み先のテーブル自体（`user_fact_items`）がB1のハイブリッド検索の対象そのもの**だった。そのため「関連する記憶をB1で検索して見せる」ことと「重複防止のために既存アイテムを見せる」ことが、偶然にも同じ操作になっていた。

しかし今回のB9（`sigmaris_entities`）・B14（`sigmaris_user_preference_patterns`）・B16（`sigmaris_goal_alignment_flags`）は、いずれも書き込み先のテーブルがB1の検索対象外（B1は`user_fact_items`しか索引していない）である。そのため「B1でuser_fact_itemsを検索して見せる」だけでは、各機能が実際に抱えていた重複排除メカニズム（`_get_or_create_entity`の`(name, entity_type)`完全一致、`_upsert_preference_pattern`の`pattern_key`完全一致、`_upsert_flag`の`goal_reference`完全一致）を直接には助けない。

このため、3箇所いずれについても**2種類のコンテキストを両方注入する設計**を採用した。

1. **B1ハイブリッド検索による関連記憶の注入**（指示書が明示的に求めていたもの）: 判断・分類の背景理解を助ける参考情報として注入する。
2. **書き込み先テーブル自身の既存アイテムを直接（検索を介さず）一覧表示する**: これが実際に重複排除メカニズムを機能させる部分。いずれのテーブルも「週次抽出のみで肥大化しない」設計（B9のコメント: 「the graph is expected to stay small」等）であるため、検索によるフィルタリングではなく全件（上限付き）表示が適切かつ、部分的な検索よりも確実だと判断した。

この判断について、指示書は「B1のハイブリッド検索を用いて...動的に検索し、注入する形に修正する」と述べており、文字通りには(2)を要求していないが、指示書はまた「各処理の性質に応じた、適切なクエリの組み立て方が採用され、判断根拠が示されること」（要件2）も求めている。書き込み先の性質（B1の索引対象外）を踏まえ、(1)のみでは当初の欠陥（重複排除メカニズムが機能していない）の直接的な解決にならないと判断し、(2)を追加した。要件4「各機能の既存の判定ロジック・出力形式自体は変更されないこと」は、(1)(2)いずれも「コンテキストとして見せる情報を増やす」のみで、判定ロジック・出力JSON形式自体は変更していないため満たしている。

### 11.2 B9（`knowledge_graph.py`）の修正

**クエリ組み立て方の判断根拠**: `extract_entities_and_relations()`はgoals・decisions・topicsの3種類のソースから抽出するバッチ処理で、単一の「直近の発言」に相当するものが存在しない。experience_layer.py（B2、7.7節・10.3節）で採用した「対象群のtitle連結」パターンを踏襲し、**このバッチが読む決定記録のtitle群と話題ラベル群を連結したもの**を検索クエリとした。goalsのvalueは含めていない（goalsは`user_fact_items`自体の一部であり、それを検索クエリにすると自分自身を検索することになり意味が薄いため）。

**既存エンティティの注入**: 既存の`get_entities_and_relations()`（B7のヒントマッチング用に既に実装済みの、上限付き全件取得関数）をそのまま再利用し、エンティティ名・種別の一覧を`_EXTRACT_PROMPT`に追加した。新しい検索ロジックは実装していない。

**フォールバック**: `jwt`が渡されない場合、または検索が失敗した場合、関連記憶コンテキストは「（なし）」に縮退するが、既存エンティティの注入（B1を介さない直接取得）はjwtの有無に関わらず機能し続ける——これは11.1節の設計上、既存エンティティの表示こそが実際の重複防止に効いている部分であるため、意図的にこちらは検索失敗の影響を受けない設計にしている。

`extract_entities_and_relations(user_id)`に`jwt: str | None = None`を追加し、`proactive/scheduler.py`の呼び出し元（既に`jwt`を取得済み）に`jwt=jwt`を追加した。

### 11.3 B14（`decision_log.py::extract_preference_patterns()`）の修正

**クエリ組み立て方の判断根拠**: 対象は`sigmaris_decision_log`の決定記録群のみ（goals/topicsは読まない）。B2と同様、**対象決定群のtitleを連結したもの**をクエリとした。

**既存パターンの注入**: 既存の`get_active_preference_patterns(limit=5)`（通常はヒント注入用に少数だけ取得する関数）を、より大きな`limit=50`で呼び出して再利用した。この関数の並び順（`evidence_count.desc, last_confirmed_at.desc`）にはクールダウン的な除外がなく、そのまま安全に再利用できると判断した。新しい取得関数は実装していない。

**フォールバック**: `jwt`/`user_id`が渡されない場合、または検索が失敗した場合、関連記憶コンテキストは「（なし）」に縮退する。この関数は元々`jwt`を一切受け取らない設計だった（`sigmaris_decision_log`・`sigmaris_user_preference_patterns`は共にservice-role専用のグローバルテーブルで、per-userのJWTを必要としなかったため）。`jwt`/`user_id`を新たにオプショナル引数として追加したため、`proactive/scheduler.py`の呼び出し元で新たに`get_sigmaris_jwt()`/`get_current_user()`を呼ぶ必要が生じた——**この呼び出し自体が失敗しても、週次バッチ全体（本来のパターン抽出処理）が失敗しないよう、jwt取得と本処理を別々のtry/exceptで分離した**（詳細は11.6節「気づいた懸念点」参照）。

### 11.4 B16（`goal_alignment.py`）の修正

**クエリ組み立て方の判断根拠**: 対象はgoals・decisions・topicsの3種類で、B9と全く同じソース構成（同一モジュールのdocstringが「B9はB16と同じ3ソースを読む」と明記している）。B9と同一の判断根拠により、**決定記録のtitle群と話題ラベル群を連結したもの**をクエリとした。

**既存フラグの注入**: 既存の`get_active_goal_alignment_flags(limit=1)`は、`_SURFACE_COOLDOWN_DAYS`（14日）以内に提示済みのフラグを除外するフィルタが掛かっており、**この関数をそのまま再利用すると、直近提示されたばかりのgoal_referenceが重複防止コンテキストから見えなくなり、まさに防ぎたいフラグメンテーションを許してしまう**。そのため、同じテーブルへの読み取りだが、クールダウンフィルタを掛けない新規の`_get_all_flags_for_context(limit=50)`を実装した。既存関数の改変（クールダウンフィルタを外す）ではなく新規関数を追加したのは、`get_active_goal_alignment_flags()`の既存の呼び出し元（応答生成時のヒント注入）の挙動を変えないためである。

**フォールバック**: `jwt`が渡されない場合、または検索が失敗した場合、関連記憶コンテキストは「（なし）」に縮退する。既存フラグの注入はjwtの有無に関わらず機能する（B9と同じ設計判断）。`extract_goal_alignment_flags(user_id)`に`jwt: str | None = None`を追加し、`proactive/scheduler.py`の呼び出し元（既に`jwt`を取得済み、B9と同型）に`jwt=jwt`を追加した。

### 11.5 テスト結果

`backend/tests/`には新規テストを追加していない（既定の方針通り、スクラッチディレクトリに作成）。6件のテストを作成し、以下を確認した:

- B9: 決定titleと話題ラベルを連結したクエリで`search_relevant_memories()`が呼ばれ、その結果と既存エンティティ一覧の両方が実際に`_EXTRACT_PROMPT`へ渡されるプロンプト文字列に反映されること。`jwt`が渡されない場合、検索は行われないが既存エンティティの注入は縮退しないこと。
- B14: 決定titleを連結したクエリで検索が呼ばれ、その結果と既存パターン一覧の両方がプロンプトに反映されること。検索失敗時に例外を投げず「（なし）」にフォールバックすること。
- B16: 検索結果と既存フラグ一覧の両方がプロンプトに反映されること。**`_get_all_flags_for_context()`が実際に送信するHTTPクエリパラメータに、`get_active_goal_alignment_flags()`が使うクールダウン除外の`or`パラメータが含まれていないことを直接assertし**、11.4節の設計判断（クールダウン非適用）が実装として正しく反映されていることを確認した。

```
6 passed in 0.73s
```

既存の`backend/tests/`（16件）、および前2タスクで作成したスクラッチテスト（`test_decision_log_relevant_facts.py`・`test_decision_type_and_b2.py`、計8件）も全て成功し、リグレッションは確認されなかった。
...
**実モデルAPIでの検証は行っていない。** 本タスクの注意事項通り、追加のサーバーアクセス・APIキー取得は試みていない。**運用者側で、次回B9・B14・B16の週次バッチが実行された後、エンティティ名・pattern_key・goal_referenceの重複が新たに発生していないか（既存の名前が正しく再利用されているか）を、数週間の運用後に確認することを推奨する。**

### 11.6 気づいた懸念点

- **B14の週次バッチが、初めてユーザー認証（jwt/user_id取得）に依存するようになった。** 修正前は`sigmaris_decision_log`・`sigmaris_user_preference_patterns`が共にservice-role専用グローバルテーブルだったため、`extract_preference_patterns()`は認証に一切依存しない設計だった。今回の修正でuser_fact_items検索のため`jwt`/`user_id`が必要になったが、**この依存を「なくてはならないもの」にしてしまうと、認証系の障害がパターン抽出機能自体を道連れにしてしまうリグレッションになる**ため、`proactive/scheduler.py`側でjwt/user_id取得と本処理呼び出しを別のtry/exceptに分離し、認証失敗時は「関連記憶コンテキストなしで抽出処理自体は続行する」形にした（11.3節）。この設計判断は、B9・B16（元々認証必須だったため新たなリグレッションはない）とは異なる対応が必要だったことを示しており、次に同様の修正を行う際は「対象機能がそれまで認証に依存していたかどうか」を必ず確認する必要があることを申し送る。
- **B9・B16双方が同一の3ソース（goals/decisions/topics）を読んでいるにもかかわらず、それぞれ独立に「決定title＋話題ラベル連結」というクエリを構築している。** これはコードの重複であり、11.7節の共通化の議論と直接関係する。
- B16の`_get_all_flags_for_context()`は`get_active_goal_alignment_flags()`とほぼ同じHTTPリクエストを行うが、パラメータ（クールダウンフィルタの有無）が異なるだけの別関数として実装した。将来的にクールダウン適用を引数化する（例: `get_active_goal_alignment_flags(limit=1, respect_cooldown=True)`）方が関数の重複を避けられたかもしれないが、既存関数のシグネチャ変更は他の呼び出し元への影響評価が必要になり、今回の限定されたスコープでは新規関数追加の方が安全と判断した。

### 11.7 「LLMへのコンテキスト供給を共通化する」リファクタリング案についての所感

実装のみ行わず、所感の記載に留める（指示通り）。

今回で、memory_extractor.py・decision_log.py（×2箇所）・experience_layer.py・knowledge_graph.py・goal_alignment.pyの**計6箇所**に、ほぼ同型の「B1検索→フォーマット→プロンプト注入→フォールバック」パターンを個別実装した。実装してみて分かった所感は以下の通り。

**共通化のメリットは明確に存在する**: 6箇所の実装は、細部の違い（クエリの組み立て方、フォールバック時の挙動、検索対象がuser_fact_itemsのみか+書き込み先テーブル自身も含むか）を除けば、コードの「形」は驚くほど似ている。特に、
- 「latest_user_text抽出 → 検索 → `- category/key: value`形式で整形 → 失敗時は「（なし）」」という部分（memory_extractor.py・decision_log.py（detect_and_record_decision）で共通）
- 「対象群のtitle/labelを連結してクエリにする → 検索 → 同じ整形 → 同じフォールバック」という部分（experience_layer.py・knowledge_graph.py・decision_log.py（extract_preference_patterns）・goal_alignment.pyで共通）

は、共通ヘルパー（例: `memory_context_helper.py`のような新規モジュールに`build_relevant_facts_context(query, *, jwt, user_id, limit)`のような1関数を切り出す）にできる余地が大きい。

**ただし、共通化を素直に行うと切り捨てられてしまう差異もある**:
- クエリの組み立て方自体は、処理ごとに本質的に異なる（単一ターンの最新発言 vs バッチのtitle連結 vs 複数ソースの連結）。共通化するとしても、この部分は各呼び出し元が個別に組み立てたクエリ文字列を渡す形にせざるを得ず、共通化できるのは「クエリを受け取ってから先」の部分のみになる。
- B9・B14・B16の3箇所は、11.1節で述べた通り「書き込み先テーブル自身の既存アイテムを直接表示する」という、B1検索とは別のロジックも必要とした。これは共通化の対象外（各テーブル固有の取得関数を使うしかない）。

**見積もり**: 6箇所全てを1つの共通ヘルパーに統合するリファクタリングは、実装自体は（今回6箇所を実装した経験を踏まえると）さほど大きくない——半日〜1日程度の作業量と見積もる。ただし、**このリファクタリングは今回の6箇所以外にも、B9で発見されたのと同型の欠陥がまだ他に残っていないかの再確認とセットで行うのが効率的**だと考える（1.1節item3で既に指摘されている「週次抽出→グローバルテーブル→TTLキャッシュ注入」という別の重複パターンの共通化とあわせて、B群全体のリファクタリングタスクとしてまとめて検討する方が、部分的な共通化を繰り返すより手戻りが少ないと考えられる）。

**推奨**: 今すぐ着手するより、まずは11.5節で述べた「運用データでの効果確認」を数週間分蓄積してから、共通化によって将来の7箇所目・8箇所目の実装コストをどれだけ下げられるかを見極めた上で、独立したリファクタリングタスクとして着手することを推奨する。
