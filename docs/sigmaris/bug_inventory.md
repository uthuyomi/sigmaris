...
- マイグレーション適用状況（「未適用」と過去に報告されたものが現在適用済みかどうか）は、リポジトリ内の`.sql`ファイル存在有無でしか確認できない。**本番DBに実際に適用されているかどうかは、この環境からは確認不能。** 運用者側での確認方法は2.1節末尾に記載。
...
2. **未適用マイグレーションの累積。** Phase A1(`202607030025`)〜B9(`202607180040`)まで、少なくとも13件のマイグレーションが各報告書時点で「未適用」と記録されている。適用順序に依存関係がある（B13のRPCがB17に依存等）。C-mini/C-fullの実測値が存在すること自体は、その後どこかの時点で運用者が適用作業を行った証拠だが、**全てが適用済みという保証はこの環境からはない**。
...
|---|---|---|---|---|---|
| 1 | BA4の統一生成アーキテクチャで`used_fallback`が常にFalse固定になり、旧来"completed_with_fallback"として吸収されていた失敗が全て"failed"として計上されるようになった可能性が高い | 本タスク2.1節（コード確認済み、実ログ未確認） | 高 | アーキテクチャ変更（BA4）に伴うフォールバック機構の消失 | 高（response_error_rate 3.6倍の直接要因候補） |
| 2 | `memory_extractor.py`の事実抽出プロンプトが既存fact一覧をLLMに見せておらず、category/key表記ゆれによる意味的重複が`user_fact_items`に蓄積し続ける構造的欠陥 | 本タスク2.2節（コード確認済み） | 高 | 設計上の欠陥（重複防止が完全一致キーのみ） | 高（memory_duplicate_rate 0.349の主因候補、memory_precisionにも波及） |
...
| 5 | decision由来テストセット0件が、decision_log蓄積不足／memory_refs未充足／参照先fact失効のいずれかまたは複合（切り分け不能） | 本タスク2.4節（コード確認済み、DB実データ未確認） | 中 | 複合要因、本番DBデータの実態確認が必要 | 中（2.4節末尾の運用者向けSQLで切り分け可能） |
...
| 8 | 少なくとも13件のマイグレーションが各報告書時点で未適用と記録されており、本番への適用状況が本セッションから確認できない | phase_b_summary.md 4章item3、他多数 | 中〜高（未適用なら機能が実質無効） | 運用フローの欠如（適用確認プロセスがない） | 高（他の全調査の前提を左右するため最優先で運用者確認を推奨） |
...
- **#1・#2は互いに独立した根本原因であり、response_error_rateとmemory_precisionの両方の悪化に波及している可能性がある**ため、優先度が最も高い。ただし#1は実ログでの`error_code`分布確認（2.1節の運用者向け手順）、#2はduplicate_clustersの実内容確認（2.2節の運用者向け手順）がまず必要で、いずれも本セッションでは実施できていない。
- **#8（マイグレーション適用状況の確認）は他の多くの調査結果の前提を左右する**ため、他の対応に先立って運用者側で確認することを推奨する。
- #4（persona_rewriter.pyのデッドコード化）は、削除するか、#1で失われたフォールバック機構として意図的に復活させるかの設計判断が必要。単純な削除では#1の根本対策にならない点に留意。
