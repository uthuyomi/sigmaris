...
## 11. `classify_chat_intent()`のnano-tier移行(2026-07-08)

対象ブランチ: `migrate-classify-intent-nano-tier`(mainからfork)。**本タ
スクは指示書の通り、mainへのマージ前に運用者の確認を得るため、ブランチ
に留めた状態で報告する。**

10章までで`classify_chat_intent()`は`max_output_tokens`上限
(9章)・クライアント再利用(9章)まで対応済みだったが、依然として生の
`AsyncOpenAI`クライアント+Responses APIを直接使用し、`local_llm.py`の
`TaskType`ルーティング(nano-tier、Ollama優先)を完全に迂回していた。本
タスクでは、この呼び出しを他のB群軽量分類処理(B7の`decompose_
query()`等)と同じ`get_llm_router()`経由のChat Completions API呼び出し
に書き換えた。

### 11.1 採用したTaskType設計

**既存の`TaskType.ROUTING`の再利用ではなく、新規の`TaskType.CHAT_
INTENT_CLASSIFICATION`を追加した。**

判断根拠: `TaskType.ROUTING`の既存利用箇所を`grep`で確認したところ、
`active_inquiry.py`(確認質問への返答解釈)・`memory_validator.py`(矛
盾検出のYES/NO判定)・`research_agent.py`・`x_content_filter.py`・
`x_post_generator.py`・`x_reply_classifier.py`という、**互いに無関係な
6つの異なる用途**が、いずれも「とりあえず軽量な判定処理だから」という
理由で`ROUTING`に相乗りしている状態だった。一方、このコードベースの支
配的な設計方針は、`DECISION_DETECTION`・`EPISODE_DETECTION`・
`TOPIC_DETECTION`・`QUERY_DECOMPOSITION`・`MEMORY_RERANK`・
`ABSTENTION_REACTION_DETECTION`のように、**意味的に異なる分類処理には
専用の`TaskType`を与える**というものであり(いずれもB群の各Phaseで新設
された)、`ROUTING`の6用途相乗りはむしろ例外的なパターンである。

`classify_chat_intent()`は、(a)本ドキュメント8〜10章にわたって専用の調
査対象になるほどホットパス上で重要度が高く、(b)6択のintent分類という、
他のいずれの既存TaskTypeとも意味的に異なる独自の判定であることから、
支配的な設計方針([各分類処理に専用のTaskTypeを与える])を踏襲し、専
用の`TaskType.CHAT_INTENT_CLASSIFICATION`を新設した。これにより、
`local_llm.py`229行目の`logger.debug("LLMRouter task=%s
backend=%s", ...)`が、今後この呼び出し単体のバックエンド選択・所要時
間を他の処理と混同せずに追跡できるようになる(8〜10章のような専用調査
が今後も必要になった場合に備えた観測性の確保)。

### 11.2 書き換えの実装詳細

**`local_llm.py`**:
- `TaskType`列挙型に`CHAT_INTENT_CLASSIFICATION = "chat_intent_
  classification"`を追加。
- `_LOCAL_TASK_TYPES`に追加(Ollamaへのローカルルーティング対象に含め
  る、要件1・3)。
- `_openai_model_for_task()`のnano階層マッピング集合に追加
  (`openai_nano_model`、要件1)。

**`chat_routing.py::classify_chat_intent()`**:
- 関数シグネチャから`client: AsyncOpenAI`・`model: str`パラメータを削
  除(呼び出し元がAPIクライアント・モデルを直接指定する必要がなくなっ
  たため)。未使用になった`from openai import AsyncOpenAI`のimportも削
  除し、代わりに`from app.services.local_llm import TaskType, get_llm_
  router`を追加。
- LLM呼び出し本体を、次のように置き換えた:

  ```python
  # 変更前(Responses API、生のクライアント直接呼び出し)
  ...
  # 変更後(Chat Completions API、TaskTypeルーティング経由)
...
- **プロンプトの内容(`prompt`変数の組み立てロジック)自体は一切変更し
  ていない。** 依頼文の要件2(「同じ情報を同じ形式で得られること」)を
  満たすため、変更対象をAPI呼び出しの伝送方式のみに絞り込み、分類精度
  に影響しうる文言変更のリスクを避けた。他のTaskTypeルーティング済み
  呼び出し(B7の`decompose_query()`等)は`system`/`user`にプロンプトを
  分割する慣習だが、今回は**あえてこの慣習には合わせず**、既存の単一
  `user`メッセージのままとした(判断根拠: プロンプトの分割自体は挙動
  上ほぼ無害と考えられるが、「トークン上限追加(9章)・API移行
  (本タスク)という2段階の変更それぞれで、プロンプト内容には一切手を
  付けない」という一貫した方針を優先した。プロンプト構造の慣習統一が
  必要であれば、分類精度への影響を単独で検証できる、独立した別タスク
  として行うのが安全と判断する)。
- `temperature=0.1`は、このコードベースの他の分類系TaskType呼び出しの
  うち、最も判定の複雑さが近いB7の`decompose_query()`
  (同じく`temperature=0.1`)に合わせた。元のResponses API呼び出しは
  temperatureを一切指定していなかった(SDKデフォルト値に依存)ため、
  **明示的な値を持たなかった状態から、より決定的な分類に適した低い値
  へ変更した**という違いがある(判断根拠: 分類タスクにおいて低
  temperatureが望ましいことは、このコードベースの他の全ての分類系呼び
  出し[0.0〜0.2の範囲]が既に示す一貫した方針であり、それに合わせた)。
- `json_mode=True`を新たに指定した(元のResponses API呼び出しは、プロ
  ンプト内の指示文("Return JSON only like...")のみに頼っており、API
  レベルでのJSON強制は行っていなかった)。これは既存の全TaskTypeルー
  ティング済み分類呼び出しが採用している標準的な設定であり、**より安
  定してJSONを返せるようになる、精度面でむしろ改善方向の変更**と考え
  られる。`_OpenAIAdapter.chat()`は`json_mode=True`時に
  `response_format={"type": "json_object"}`を付与するが、これはプロン
  プト中に"json"という語が含まれていることを要求するOpenAI APIの制約
  があり、既存プロンプトの`'Return JSON only like {"intent":"...",
  "reason":"..."}.'`という一文が既にこの条件を満たしていることを確認
  済みである。
- 800というトークン上限の値自体は変更していない(要件4)。ただし、9章
  で800という値を選んだ根拠(Responses APIの推論モデルでは、推論トー
  クンも含めた合計に上限がかかるため、厳しくしすぎるリスクがある)は、
  Chat Completions API+nano階層モデルには同程度には当てはまらない可能
  性が高いと考えられる(このコードベースの他のnano階層TaskType呼び出
  しは、いずれも100〜300程度のより小さい上限で問題なく運用されてい
  る)。今回は要件4に従い値そのものは変更しなかったが、実測で妥当性を
  確認できれば、将来的に800より小さい値へ調整する余地があることを
  11.6節に申し送る。

**`chat.py`**: `run_chat_completion()`・`stream_chat_completion_ui()`の
両呼び出し箇所(旧492〜498行目・767〜773行目)から、`classify_chat_
intent()`への`client=`・`model=`引数を削除した(呼び出し方法のみの変
更で、`client = _require_openai_client()`自体は本体応答生成のために引
き続き必要なため削除していない)。

### 11.3 Ollamaフォールバックの動作確認結果

要件3(Ollama利用不可時、404を待たずに即座にOpenAI nanoへフォールバッ
クすること)について: **この挙動は、`local_llm.py`の既存の`LLMRouter`
機構にそのまま乗ることで、新規のコードを一切書かずに満たされる**こと
を確認した。

- `LLMRouter._get_backend()`(200〜216行目)は、対象TaskTypeが`_LOCAL_
  TASK_TYPES`に含まれ、かつ`settings.local_llm_enabled`が真の場合の
  み、`is_available()`(10章で確認した、単なる到達性だけでなくチャッ
  ト用モデルの有無まで確認する実装)を**プロセス生存期間中に1回だけ**
  呼び出し、結果を`self._local_available`にキャッシュする。以降の全て
  の呼び出しは、この1回のキャッシュ結果を参照するだけであり、Ollamaへ
  の実際のPOSTを試みて404を待つ、という経路には一切入らない
  (`_get_backend()`はバックエンドの選択のみを行い、選ばれなかった側へ
  のフォールバック的な再試行は行わない設計)。
- `TaskType.CHAT_INTENT_CLASSIFICATION`を`_LOCAL_TASK_TYPES`に追加した
  ことで、この呼び出しは既存の他のnano階層呼び出しと全く同じ経路を通
  るようになった。10章の調査時点でOllamaのチャット用モデルが利用不可
  と判定されている場合、`classify_chat_intent()`も含め、この判定以降
  の全ての local-eligible TaskType呼び出しが、404を待つことなく即座に
  `_OpenAIAdapter`(nano階層)へルーティングされる。
- モックテスト(11.4節)で、実際にこの経路(`LocalLLMClient.chat()`が
  一切呼ばれないこと、`is_available()`が複数回の呼び出しにわたって1回
  しか実行されないこと)を直接検証した。

### 11.4 テスト結果

`backend/tests/`には未コミット(既存方針に合わせスクラッチテストとして
実行のみ)。実モデルAPI・実Supabase接続・実Ollamaへのアクセスは行って
いない。

**`chat_routing.classify_chat_intent()`の配線検証(6件)**:
- LLM呼び出しが`TaskType.CHAT_INTENT_CLASSIFICATION`・
  `max_tokens=800`・`json_mode=True`で`get_llm_router().chat()`に渡さ
  れることの直接確認(要件1・4)
- 旧`client=`/`model=`引数を渡すと`TypeError`になることの確認(シグネ
  チャが確実に変更されたことの回帰確認)
- ヒューリスティックでは判定しきれない発言を経由したLLM分類が、
  `calendar_write`のような`general_chat`以外のintentも引き続き正しく
  返せることの確認(要件2)
- ヒューリスティック経路が、変更後も**ルーターを一切呼ばずに**即座に
  判定できることの回帰確認(要件5)
- 空・非dict出力時に、既存の`intent not in VALID_INTENTS`フォールバッ
  クで安全に`general_chat`へ縮退することの確認
- ルーター呼び出し自体が例外を送出した場合も、`source=fallback`で安全
  に縮退することの確認

**`TaskType.CHAT_INTENT_CLASSIFICATION`自体の設定検証(2件)**:
- `_openai_model_for_task()`が`settings.openai_nano_model`を返すことの
  確認(要件1)
- `_LOCAL_TASK_TYPES`に含まれていることの確認(要件1・3)

**`LLMRouter`の実ルーティング検証(4件、`chat_routing.py`を介さず
`local_llm.py`のルーター自体を直接検証)**:
- Ollamaにチャット用モデルがある(`is_available()`が真を返す)場合、
  ローカル`LocalLLMClient.chat()`が呼ばれ、OpenAI側は一切呼ばれないこ
  との確認
- Ollamaにチャット用モデルがない(`is_available()`が偽を返す)場合、
  ローカル`LocalLLMClient.chat()`を**一度も呼ばずに**、即座にOpenAI
  nano側へフォールバックすることの確認(要件3の直接検証。ローカル
  `chat()`が呼ばれたら即座に失敗するモックにして確認)
- 複数回・複数TaskTypeにわたって呼び出しても、`is_available()`(疎通
  確認)が1回しか実行されないことの確認(10章で確認した「プロセス生存
  期間中1回」という設計が、この新規TaskTypeでも維持されていることの確
  認)
- `LOCAL_LLM_ENABLED=false`の場合、疎通確認自体を一切行わずOpenAIへ直
  行することの確認

```
16 passed (既存の回帰テスト一式、backend/tests/、変更なし)
2 passed  (chat.py::_require_openai_client、9章の既存テスト再実行、
           本タスクでは変更していないことの確認)
6 passed  (chat_routing.classify_chat_intent配線検証)
2 passed  (CHAT_INTENT_CLASSIFICATIONのTaskType設定検証)
4 passed  (LLMRouterの実ルーティング検証)
= 30 passed(本タスク関連の合計)
```

**既存機能への非破壊確認**: 上記に加え、Phase BA1〜BA2の既存スクラッチ
テスト(単体テストのみ、BA1のorchestrator統合テスト3件は除外)を再実
行し、全て成功することを確認した。BA1のorchestrator統合テスト3件
(`InquiryFireAndForgetIntegrationTests`等)については、**本タスクとは
無関係に、BA4が`orchestrator/service.py`から`rewrite_with_persona_
stream`を削除したことに起因して、mainの現時点でも既に失敗する状態
だった**ことを、本タスクの変更を`git stash`で一時的に取り除いた上で確
認した(mainのままでも同じ3件が同じ理由で失敗する)。これは本タスクが
持ち込んだ回帰ではなく、BA1セッション由来の未コミットスクラッチテスト
が、BA4後にAPIごと古くなっていたことによるものであり、本タスクの範囲
外の問題として付記するに留める。

```
54 passed (backend/tests/ + Phase BA1単体テスト + Phase BA2全テスト +
           本タスクのテスト、BA1の失効した統合テスト3件を除く)
```

### 11.5 レイテンシへの影響(見積もり、実測は未実施)

指示書の制約(新規のAPIキー・サーバーアクセスを取得しない)に従い、実
モデルAPIでの前後比較測定は行っていない。以下はコードから導かれる見積
もりであることを明記する。

- **Ollamaのチャット用モデルが利用可能になった場合**: ネットワークを介
  さないローカル推論になるため、9章時点の約3.7秒から、大幅な短縮
  (数百ミリ秒程度、他のローカル推論呼び出しと同等のオーダー)が期待
  できる。ただしこれは10章で優先度1とした「Ollama側の設定確認・修正」
  (本タスクの範囲外、インフラ側の対応)が別途行われた場合に限る。
- **Ollamaが利用不可のまま(現状)の場合**: OpenAI nano階層
  (`openai_nano_model`)への呼び出しになる。9章時点の"mini"階層
  (`openai_model`)より軽量なモデルであり、かつ`json_mode=True`によ
  る出力の安定化も加わるため、**9章の3.7秒からさらに短縮される可能性
  が高い**が、具体的にどの程度かは実測なしには断定できない。参考とし
  て、このコードベースの他のnano階層・Chat Completions API呼び出し
  (B7の`decompose_query()`)は、10章の実測で約2.0〜2.5秒
  (Ollama不可時のOpenAIフォールバック込み)というオーダーだった。
  `classify_chat_intent()`のプロンプト長・複雑さは`decompose_query()`
  と概ね近いため、**同程度のオーダー(3.7秒より短く、数秒未満)まで縮
  まる可能性がある**というのが、現時点での最も根拠のある見積もりであ
  る。

**次回、本番サーバーで実測ログを取得する際は、`classify_chat_intent()`
区間の所要時間が、9章時点(約3.7秒)・8章時点(約8.3秒)と比べてどう変
化したかを重点的に確認することを推奨する。**

### 11.6 気づいた懸念点

- **トークン上限(800)の再検討余地**: 11.2節で述べた通り、Chat
  Completions API+nano階層では、Responses API+mini階層ほど「推論トー
  クンが上限を食い潰す」リスクは高くないと考えられる。要件4に従い今回
  は800のまま維持したが、実測で妥当性を確認できれば、他のnano階層呼び
  出し(100〜300程度)に近い値へ調整し、さらなる短縮を図る余地があ
  る。
- **プロンプトのsystem/user分割の不統一**: 11.2節で述べた通り、今回は
  安全側に倒してプロンプトの分割を行わなかったため、`classify_chat_
  intent()`は他のTaskTypeルーティング済み呼び出し(B7・B14・B15・B16
  等の多くがsystem/userに分割している)と構造が異なったままである。将
  来的な統一を検討する場合は、分類精度への影響を独立して検証できる、
  専用のタスクとして行うことを推奨する。
- **`TaskType.ROUTING`の6用途相乗り状態は今回解消していない**:
  11.1節で確認した既存の重複利用(`active_inquiry.py`・`memory_
  validator.py`・`research_agent.py`・`x_content_filter.py`・
  `x_post_generator.py`・`x_reply_classifier.py`)は、本タスクの範囲
  外として一切手を付けていない。これらも将来的に専用TaskTypeへ分離す
  る価値があるかもしれないが、緊急性は低いと判断し、申し送りに留め
  る。
- **10章の優先度1(Ollamaのチャット用モデル設定)が未解決のままだと、
  本タスクの効果は限定的**: 本タスクは「nano階層への移行」というコー
  ド側の対応を完了させたが、Ollama自体が利用可能にならない限り、実際
  に得られる短縮効果は「mini→nanoモデルへの変更」分にとどまる
  (10.5節で見積もった通り、ローカル推論が使えるようになった場合の効
  果の方がはるかに大きい)。両者は独立に対応可能だが、**最大の効果を
  得るには両方が必要**であることを改めて強調しておく。
- **未コミットのBA1スクラッチテストの陳腐化を発見**(11.4節): 本タス
  クの範囲外だが、`InquiryFireAndForgetIntegrationTests`等3件が、
  BA4による`rewrite_with_persona_stream`削除以降、mainの現状でも既に
  失敗する状態になっていることに気づいた。これらは元々`backend/
  tests/`にコミットされていない、セッションローカルのスクラッチテスト
  であるため実害はないが、今後BA1の統合テストを再度必要とする機会が
  あれば、書き直しが必要になる。
